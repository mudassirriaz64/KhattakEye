...
const uniqueBrands = brands.reduce<{ name: string; logo: string }[]>((acc, brand) => {
  if (!acc.some((b) => b.name === brand.name)) acc.push(brand);
  return acc;
}, []);
          ...
          {uniqueBrands.map((brand, index) => (
            ...
            >
              <div className="relative">
                ...
                <span className={`absolute -bottom-1 -right-1 h-6 w-6 rounded-full ${accents[index % 4].tint} ring-4 ring-[color:var(--color-panel)]`} />
              </div>
              ...
              <p className="mt-2 text-xs uppercase tracking-[0.22em] text-[color:var(--color-text-tertiary)]">
                {index === 0 ? "Sculptural" : index === 1 ? "Distinctive" : index === 2 ? "Timeless" : "Engineered"}
              </p>
            </motion.div>
          ))}
        </div>
