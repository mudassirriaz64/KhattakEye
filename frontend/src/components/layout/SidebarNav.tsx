...
import { Glasses, LayoutGrid, Sparkles } from "lucide-react";
...
export function SidebarNav() {
  ...
  return (
    <aside className="hidden w-[290px] shrink-0 flex-col border-r border-[color:var(--color-border)] bg-[color:var(--color-panel)] px-5 py-6 lg:flex">
      ...
      <div className="mt-auto rounded-[22px] border border-[color:var(--color-border)] bg-[color:var(--color-surface-muted)] px-4 py-4">
        <div className="flex items-center gap-3">
          <Sparkles className="h-4 w-4 text-[color:var(--color-brand-primary)]" />
          <div>
            <p className="text-sm font-medium text-[color:var(--color-text-primary)]">Khattak Luxury</p>
            <p className="text-xs text-[color:var(--color-text-secondary)]">Single light theme</p>
          </div>
        </div>
      </div>
    </aside>
