...
import { Heart, ShoppingBag, Star, Eye, GitCompare, Glasses, Zap } from "lucide-react";
  ...
  return (
    <>
      <motion.div
        ...
        exit={{ opacity: 0 }}
        className={cn(
          "group relative overflow-hidden rounded-2xl border border-[color:var(--color-border)] bg-[color:var(--color-panel)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[var(--shadow-soft)]",
          isList ? "flex" : "",
        )}
      >
        <Link to={`/product/${product.slug}`} onClick={handleClick} className={cn("relative overflow-hidden", isList ? "w-56 shrink-0" : "aspect-[4/5]")}>
          <div className={cn("h-full w-full", isList ? "" : "aspect-[4/5]")}>
            <img
              ...
              alt={product.name}
              className="h-full w-full object-cover transition-all duration-500 ease-out group-hover:scale-105 group-hover:opacity-0"
            />
            {product.hoverImage && (
              <img
          ...
          <div className="absolute inset-x-0 bottom-0 flex translate-y-full items-center justify-center gap-2 bg-gradient-to-t from-black/60 to-transparent p-4 opacity-0 transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-100">
            <Link
              to={`/virtual-try-on?product=${product.slug}`}
              onClick={handleClick}
              className="flex items-center gap-1.5 rounded-full bg-white/20 px-4 py-2 text-[10px] font-semibold text-white backdrop-blur transition-colors hover:bg-white/30"
            >
        ...
        <div className={cn("flex flex-1 flex-col p-4", isList ? "justify-center" : "")}>
          ...
          <div className={cn("mt-auto flex gap-2", isList ? "mt-4" : "")}>
            <Link to={`/product/${product.slug}`} onClick={handleClick} className="flex-1">
              <button
                type="button"
                className="flex w-full items-center justify-center gap-1.5 rounded-xl bg-[color:var(--color-brand-primary)] px-3 py-2.5 text-xs font-semibold text-white transition-all hover:bg-[#19130D]"
              >
            ...
            <button
              ...
              className="flex items-center gap-1.5 rounded-xl bg-[color:var(--color-accent-teal)] px-3 py-2.5 text-xs font-semibold text-white transition-all hover:bg-[#8F1215]"
            >
