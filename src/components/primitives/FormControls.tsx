...
const fieldBaseClass =
  "w-full rounded-[16px] border border-[color:var(--color-border)] bg-white/90 px-4 py-3 text-sm text-[color:var(--color-text-primary)] shadow-[var(--shadow-input)] outline-none transition-all placeholder:text-[color:var(--color-text-tertiary)] hover:border-[color:var(--color-border-strong)] focus:border-[color:var(--color-accent-blue)] focus:ring-4 focus:ring-[color:var(--color-focus-ring)] dark:bg-white/5";
...
export function TextField({
  ...
  return (
    <label className="space-y-2">
      <span className="text-[13px] font-semibold uppercase tracking-[0.16em] text-[color:var(--color-text-secondary)]">
        {label}
      </span>
...
export function CheckboxField({ label, helper, error }: FieldProps) {
  return (
    <label className="flex items-start gap-3 rounded-[18px] border border-[color:var(--color-border)] bg-[color:var(--color-surface-muted)] p-4">
      <input type="checkbox" className="mt-1 h-4 w-4 rounded border-[color:var(--color-border-strong)]" defaultChecked />
...
export function ToggleField({ label, helper }: Pick<FieldProps, "label" | "helper">) {
  return (
    <div className="flex items-center justify-between rounded-[18px] border border-[color:var(--color-border)] bg-[color:var(--color-surface-muted)] p-4">
      ...
      <button
        type="button"
        className="flex h-8 w-14 items-center rounded-full bg-[color:var(--color-brand-primary)] px-1"
        aria-pressed="true"
      >
