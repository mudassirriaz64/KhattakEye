      ...
      {activeTab === "conversion" && (
        <div className="grid gap-6 lg:grid-cols-2">
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl border border-[color:var(--color-border)] bg-[color:var(--color-panel)] p-5">
            ...
            <div className="mt-6">
              <DonutChart data={conversionMetrics.map((c) => ({ label: c.source, value: c.conversions, color: c.source === "Direct" ? "var(--color-brand-primary)" : c.source === "Organic Search" ? "var(--color-accent-teal)" : c.source === "Social Media" ? "var(--color-accent-blue)" : c.source === "Email" ? "#f59e0b" : "#8b5cf6" }))} />
            </div>
          </motion.div>
